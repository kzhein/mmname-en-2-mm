...
const LANG_MY = 0; // Myanmar

const MM_MAX_CONTEXT_LENGTH = 4;

function get_char_class(char) {
  let identifiedClass = MY_SYLLABLE_UNKNOWN;
...
function get_break_status(before, after) {
  ...
  const firstClass = get_char_class(before);
  const secondClass = get_char_class(after);

  return BKSTATUS[firstClass][secondClass];
}

function evaluate_context(contextText, offset, langHint) {
  let text = contextText.substring(offset, contextText.length);
...
function get_next_syllable(text, length, offset) {
  let breakType = MY_PAIR_NO_BREAK;
  let i = offset;
...
function is_not_myanmar(char) {
  const charClass = get_char_class(char);
  if (
    charClass == MMC_OT ||
    charClass == MMC_RQ ||
    charClass == MMC_LQ ||
...
export function syllable(input) {
  let offset = 0;
  const result = [];
